...
with open(path.join(FILE_DIR, 'lwe_plugin_shell', 'version.py')) as f:
    version = re.match(r'^__version__ = "([\w\.]+)"$', f.read().strip())[1]

setup(
    name="lwe-plugin-shell",
    version=version,
    author="Chad Phillips",
    author_email="chad@apartmentlines.com",
    description="LWE plugin: Shell plugin",
    ...
    entry_points={
        "lwe_plugins": [
            "lwe_plugin_shell = lwe_plugin_shell.plugin:Shell"
        ]
    },
)
